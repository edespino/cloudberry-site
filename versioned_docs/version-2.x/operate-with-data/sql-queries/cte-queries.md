...
By default, Apache Cloudberry enables the `RECURSIVE` keyword for the `WITH` clause. `RECURSIVE` can be deactivated by setting the server configuration parameter `gp_recursive_cte` to `false`.
...
- As long as the working table is not empty, repeat these steps:
    ...
    :::note
    While `RECURSIVE` allows queries to be specified recursively, Apache Cloudberry evaluates such queries iteratively internally.
    :::
...
Aside from detecting cycles, the array value is useful in its own right because it represents the "path" taken to reach any particular row.
...
In particular, if there's an index on `key`, Apache Cloudberry uses it to fetch just the rows having `key = 123`. On the other hand, in:
...
Here, materialization of the `WITH` query ensures that Apache Cloudberry evaluations `very_expensive_function` only once per table row, not twice.
...
- Apache Cloudberry inlines multiply-referenced CTEs only when requested (by specifying `NOT MATERIALIZED`).

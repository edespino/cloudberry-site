---
title: Test and Debug Text Search
---

# Test and Debug Text Search

This document introduces the Apache Cloudberry functions you can use to test and debug a search configuration or the individual parser and dictionaries specified in a configuration.
...
For a more extensive demonstration, first create a `public.english` configuration and Ispell dictionary for the English language:
...
The word `The` was recognized by the `english_ispell` dictionary as a stop word ([Stop Words](./text-search-dictionaries.md#stop-words)) and will not be indexed. The spaces are discarded too, because the configuration provides no dictionaries at all for them.
...
The thesaurus dictionary `thesaurus_astro` does know the phrase `supernovae stars`, but `ts_lexize` fails because it does not parse the input text but treats it as a single token. Use `plainto_tsquery` or `to_tsvector` to test thesaurus dictionaries, for example:

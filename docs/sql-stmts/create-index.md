...
Additional restrictions apply when unique indexes are applied to partitioned tables; see [`CREATE TABLE`](/docs/sql-stmts/create-table.md).
...
The system regularly collects statistics on all of a table's columns. Newly-created non-expression indexes can immediately use these statistics to determine an index's usefulness. For new expression indexes, you must run [ANALYZE](/docs/sql-stmts/analyze.md) to generate statistics for these indexes.
...
Use [`DROP INDEX`](/docs/sql-stmts/drop-index.md) to remove an index.

...
> **Note** A view that is simple enough to be automatically updatable (see [CREATE VIEW](/docs/sql-stmts/create-view.md)) does not require a user-created rule in order to be updatable. While you can create an explicit rule anyway, the automatic update transformation will generally outperform an explicit rule.
...
[ALTER RULE](/docs/sql-stmts/alter-rule.md), [DROP RULE](/docs/sql-stmts/drop-rule.md)

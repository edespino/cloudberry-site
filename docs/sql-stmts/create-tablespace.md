...
If a segment instance is not listed in the `WITH` clause, Apache Cloudberry uses the tablespace directory specified in the `LOCATION` clause.
...
Because `CREATE TABLESPACE` creates symbolic links from the `pg_tblspc` directory in the coordinator and segment instance data directory to the directories specified in the command, Apache Cloudberry supports tablespaces only on systems that support symbolic links.
...
`CREATE TABLESPACE` is a Apache Cloudberry extension.
